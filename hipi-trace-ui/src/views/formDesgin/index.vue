<template>
  <a-layout id='components-layout-demo-fixed-sider'>
    ...
    <a-layout :style="{ marginLeft: '-30px' }">
      <a-layout-header class='center-header'>
        ...
        <a-divider type='vertical' />
        <a-button icon='delete' size='small' type='link' @click='cancel'>
          关闭
        </a-button>
      ...
      <div class='center-main'>
        ...
        <a-layout-footer :style="{ textAlign: 'center' }">
          ©2022 <a href='https://github.com/fuzui/form-generator-antdv' target='_blank'>Form Generator Antdv2</a>
        </a-layout-footer>
      </div>
    </a-layout>
    <a-layout-sider
...
import { inputComponents, layoutComponents, selectComponents } from '@/components/generator/config'
import { beautifierConf, deepClone, isObjectObject, stringify, titleCase } from '@/utils/index'
...
import { DRAWING_ID, DRAWING_ITEMS } from '@/store/mutation-types'
...
export default {
  ...
  mounted() {
    ...
    // }
    console.log(this.userForm, 'lalalal')
    if (JSON.stringify(this.userForm) != '{}' && JSON.stringify(this.userForm) != '') {
      this.drawingList = this.userForm.fields
    }
  ...
  methods: {
    ...
    onEnd(obj) {
      if (obj.from !== obj.to) {
        console.log('onEnd::', tempActiveData)
    ...
    drawingItemCopy(item, list) {
      ...
      console.log('drawingItemCopy::', clone)
      list.push(clone)
      this.activeFormItem(clone)
    },
    drawingItemDelete(index, list) {
      console.log('drawingItemDelete index::', index)
      list.splice(index, 1)
      this.$nextTick(() => {
